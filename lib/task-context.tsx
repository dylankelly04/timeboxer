...
export function TaskProvider({ children }: { children: ReactNode }) {
  ...
  const addTask = useCallback(async (task: Omit<Task, "id">) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error("Error adding task - API response:", response.status, errorData)
  ...
  const moveTaskToDate = useCallback(async (taskId: string, newDate: Date) => {
    const newDateStr = format(newDate, "yyyy-MM-dd")
    await updateTask(taskId, { startDate: newDateStr, scheduledTime: undefined })
  }, [updateTask])
